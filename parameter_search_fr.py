...
if __name__ == '__main__':
    ...
    parser.add_argument('-a', '--alphas',
                        ...
                        default=[0.1, 0.2, 0.4, 0.6, 0.9])
    parser.add_argument('-bs', '--batch_sizes',
                        help="batch sizes for hyperparameter search (default: [20, 50, 80, 100, 200]",
                        nargs='+',
                        type=int,
                        default=[200, 100, 80, 50, 20])
    parser.add_argument('-src_vecs',
                        help="source embedding file (default: embeddings/original/google.bin)",
                        default='embeddings/original/google.bin')
    parser.add_argument('-trg_vecs_dir',
                        help='directory of target language vectors. Vectors should be named sg-100-LANG-.bin, where LANG is the two letter language identifier, i.e. ES for Spanish',
                        default='embeddings/original/sg-100-fr.bin')
    ...
    for lang in args.langs:
        print('importing target word embeddings')
        # trg_vec_file = os.path.join(args.trg_vecs_dir, 'sg-300-{0}.bin'.format(lang))
        # trg_vec_file = 'embeddings/col_fr/model.txt'
        trg_vec_file = 'embeddings/fr/fr.bin'
        trg_vecs = WordVecs(trg_vec_file, 'fr', 'bin')

        # Import monolingual vectors
        print('importing source word embeddings')
        # src_vec_file = 'embeddings/col_en/model.txt'
        src_vecs = WordVecs(args.src_vecs, 'google', 'bin')
        # src_vecs = WordVecs(src_vec_file, 'file', 'word2vec')
